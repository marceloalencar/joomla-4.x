...
choices.clearInput();});const setNewCategory=(()=>{const new_category=input.value;console.log('----');console.log(input.value);console.log(select.value);choices.clearInput();if(!new_category.length){return;}
const new_option=new Option(new_category,new_category);select.add(new_option);select.value=new_category;choices._addChoice(new_option);choices._triggerChange(new_category);choices.setChoiceByValue(new_category);choices.clearInput();console.log('----');console.log(new_category);});});})();