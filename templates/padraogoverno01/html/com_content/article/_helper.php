...
class TemplateContentArticleHelper {
	...
	static function displayCategoryLinks( $parent_categories, $item )
	{
		...
		$url = '<span><a class="link-categoria" rel="tag" href="'.JRoute::_(ContentHelperRoute::getCategoryRoute($item->catid)).'">'.$item->category_title.'</a></span>'; 
	...
	static function displayFulltextImage( $images = NULL, $params = NULL )
	{
		...
		if (isset($images->image_fulltext) and !empty($images->image_fulltext)):
			...
			if(strpos($images->image_fulltext, 'www.youtube') !== false):
				...
				if(is_file(JPATH_SITE.DIRECTORY_SEPARATOR.$images->image_fulltext)):
					list($width, $height, $type, $attr) = getimagesize( JPATH_SITE.DIRECTORY_SEPARATOR.$images->image_fulltext );
				else:
			...
			elseif($type=='lightbox'):
				...
				if(@isset($img_modal))
				{
					if( !is_file(JPATH_SITE.DIRECTORY_SEPARATOR.$img_modal) )
						$img_modal = false;
				}
				else
					$img_modal = false;
